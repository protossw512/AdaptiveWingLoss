...
class AddCoordsTh(nn.Module):
    ...
    def forward(self, input_tensor, heatmap=None):
        ...
        do_cuda = input_tensor.is_cuda

        xx_ones = torch.ones([1, self.y_dim], dtype=torch.int32)
        xx_ones = xx_ones.cuda() if do_cuda else xx_ones
        xx_ones = xx_ones.unsqueeze(-1)

        xx_range = torch.arange(self.x_dim, dtype=torch.int32).unsqueeze(0)
        xx_range = xx_range.cuda() if do_cuda else xx_range
        ...
        yy_ones = yy_ones.cuda() if do_cuda else yy_ones
        yy_ones = yy_ones.unsqueeze(1)

        yy_range = torch.arange(self.y_dim, dtype=torch.int32).unsqueeze(0)
        yy_range = yy_range.cuda() if do_cuda else yy_range
        ...
        if self.with_boundary and type(heatmap) != type(None) and do_cuda:
